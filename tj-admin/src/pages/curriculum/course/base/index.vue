...
const handleSubmit = async (str) => {
  ...
  if (fromData.value.coverUrl !== undefined && fromData.value.coverUrl !== "") {
    flag.value = true;
  }
  console.log('提交前的表单数据:', fromData.value); // 调试日志
  console.log('coverUrl值:', fromData.value.coverUrl); // 重点检查coverUrl
...
const getCoverUrl = (val) => {
  console.log('获取到的封面URL:', val); // 调试日志
  if (val) {
    fromData.value.coverUrl = val;
    flag.value = true; // 确保flag也被设置
  }
};
