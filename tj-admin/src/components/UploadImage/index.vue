...
const handleAvatarSuccess = (response, uploadFile) => {
  ...
  // 情况1: 有统一响应包装 { code: 200, data: { path: "xxx" } }
  if (response.data && typeof response.data === 'object' && response.data.path) {
    imagePath = response.data.path;
    console.log('✅ 从 response.data.path 获取:', imagePath);
  }
  // 情况2: 直接返回 FileDTO { id: xx, path: "xxx", filename: "xx" }
  else if (response.path) {
    imagePath = response.path;
    console.log('✅ 从 response.path 获取:', imagePath);
  }
  // 情况3: 其他可能的字段
  else if (response.data && response.data.url) {
    imagePath = response.data.url;
    console.log('✅ 从 response.data.url 获取:', imagePath);
  }
  else if (response.url) {
    imagePath = response.url;
    console.log('✅ 从 response.url 获取:', imagePath);
  }
  
  if (imagePath) {
    console.log('✅ 最终图片路径:', imagePath);
    emit("getCoverUrl", imagePath);
  } else {
    console.error('❌ 无法从响应中获取图片路径');
    ElMessage.error('图片上传成功但未能获取到图片路径，请检查控制台日志');
  }
}
// 获取上传的图片路径
